...
Functions:
    validate_db_path(db_path): Check validity of database path.
    validate_data_types_path(data_types_path): Check data types path.
    ...
    build_year_tables(): Create year tables.
    build_table(table_name): Create tables and add appropriate columns.
    sanitize(string): Check strings for characters that could harm database.
    copy_table(source_table, target_table): Copy an existing table.
"""
import os
import json
import sqlite3
...
def validate_data_types_path(data_types_path):
...
def initialize_database(db_path, data_types_path):
    ...
    validate_data_types_path(data_types_path)
    global DB_PATH
    DB_PATH = db_path
